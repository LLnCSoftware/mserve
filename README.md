...
The demo server is a stand-in for a production api server. It has just 2 endpoints, "proc1" and "proc2". 
...
However proc2 was made more expensive than proc1 by executing the query more times.
...
Unfortunately, "Secure Invocation" prevents mserve from setting the [.z.ps](https://code.kx.com/q/ref/dotz/#zps-set) handler to cause
...
Originally mserve\_np just sent each query to the first servant from the top the the list which was not busy.
...
1. A **request** is a general list where the first element is an integer id 
 intended to be returned back with the response, and the second element 
 is the query (i.e., command) either as as string or general list.
...
The query can be a generalized list:
```
 (function_defined_in_the_servant; arg1; arg2; arg3...)
```


2. A **response** is a general list where the first element is an integer id
 returned from the request, and the second element is the result computed by the servant. 
...
## Secure Invocation
 ...
 See: [Processing Messages](https://code.kx.com/q4m3/11_IO/#1165-processing-messages) for a discussion of 
 safe remote calls. 
...
- **Predictable Behavior:** Relies on predefined functions or controlled execution paths.
- **Improved Security:** Reduces the attack surface for injection or unintended behavior.

#### Example
```q
// Risky: evaluating an arbitrary string
evalString:{@[.z.p;enlist x;{`error$x}]}
